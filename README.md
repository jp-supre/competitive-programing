...
この解説はわかりやすい
http://baitop.hatenadiary.jp/entry/2020/01/24/005811

今回は、人は区別するが、箱（グループ）に区別はない。
（同じ人数のグループに区別がないということ）

この時、答えは、A<=i<=B,C<=j<=Dで、

$$\begin{aligned} [x^n]n!\prod_i(1 + \sum_j \frac{1}{(i!)^jj!}x^{ij}) \end{aligned}$$

n!でn人の人を並べ替えて、i人のグループがj個ある時、
その一つ一つのグループ内のi人のi!通りの並べ替えは区別しない。
それがj個あるから、i!^jで、さらに、同じ人数のグループにも区別がないから、
j!で割る。という感じ。
...
ll func(string s) {
    ...
    rep(i,n) {
        ...
        rep(j,2) {
            rep(k,2) {
                for (int d=0;d<=(j?9:now);d++) {
                    dp[i+1][j||(d<now)][k||(d==4)||(d==9)]+=dp[i][j][k];
                }
            }
        }
    }
...
int main() {
    ...
    cout<<func(toString(b))-func(toString(a-1))<<endl;
...
似た感じだけど、こっちは2進数として扱う。
ある桁を0か１か決めていくイメージ。
0にしたときは、(配列Aの中でその桁が1になってるやつの個数)*2^(桁数)
1のときは、(配列Aの中でその桁が0になってるやつの個数)*2^(桁数)
を足していく感じで、最大値を持ったdpを更新していく。
<details><summary>コード</summary><div>
